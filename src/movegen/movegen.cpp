...
void Movegen::pawn_move(std::vector<Move>& moves, uint8_t from, uint8_t to, PAWN_FLAG flag) {

    uint8_t promote_flag = 0;
    if(flag == PAWN_FLAG::PROMOTION) {
        for(uint8_t i = 0; i < 4; i++) {
            promote_flag = i;
            moves.push_back(Move::encode(from, to, promote_flag, static_cast<uint8_t>(PAWN_FLAG::PROMOTION)));
        }
    } else if(flag == PAWN_FLAG::EN_PAS) {
        moves.push_back(Move::encode(from, to, 0, static_cast<uint8_t>(PAWN_FLAG::EN_PAS)));
    } else {
        moves.push_back(Move::encode(from, to, 0, static_cast<uint8_t>(PAWN_FLAG::NONE)));
    }
}
...
void Movegen::generate_pawn_moves(Position& pos, PIECE_C color, std::vector<Move>& moves) {
    ...
    while(bitboard) {
        ...
        if(color == PIECE_C::WHITE) {
            ...
            PAWN_FLAG flag = PAWN_FLAG::NONE;
            if(from_bb & rank7) {
                flag = PAWN_FLAG::PROMOTION;
            }

            if(push_bb & empty) {
                pawn_move(moves, sq, sq + 8, flag);

                if((from_bb & rank2) && ((push_bb << 8) & empty)) {
                    pawn_move(moves, sq, sq + 16, PAWN_FLAG::NONE);
                }
            }
            if(left_attack_bb & enemy_bb) {
                pawn_move(moves, sq, sq + 7, flag);
            }
            if(left_attack_bb & en_pas_bb) {
                pawn_move(moves, sq, sq + 7, PAWN_FLAG::EN_PAS);
            }
            if(right_attack_bb & enemy_bb) {
                pawn_move(moves, sq, sq + 9, flag);
            }
            if(right_attack_bb & en_pas_bb) {
                pawn_move(moves, sq, sq + 9, PAWN_FLAG::EN_PAS);
            }
        } else {
            auto push_bb = from_bb >> 8;
